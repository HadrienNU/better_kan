    ...
    def __new__(cls, *args, **kwargs):
        if cls is LinearOperator:
            ...
            if type(obj)._matvec == LinearOperator._matvec and type(obj)._matmat == LinearOperator._matmat:
                warnings.warn("LinearOperator subclass should implement" " at least one of _matvec and _matmat.", category=RuntimeWarning, stacklevel=2)
...
class _CustomLinearOperator(LinearOperator):
    ...
    def _adjoint(self):
        return _CustomLinearOperator(
            shape=(self.shape[1], self.shape[0]), matvec=self.__rmatvec_impl, rmatvec=self.__matvec_impl, matmat=self.__rmatmat_impl, rmatmat=self.__matmat_impl, dtype=self.dtype
        )
...
class ConcatLazy(LinearOperator):
    ...
    def __init__(self, Ms):
        self.Ms = Ms
        assert all(M.shape[0] == Ms[0].shape[0] for M in Ms), f"Trying to concatenate matrices of different sizes {[M.shape for M in Ms]}"
        shape = (sum(M.shape[0] for M in Ms), Ms[0].shape[1])
        super().__init__(None, shape)

    def _matmat(self, V):
        return np.concatenate([M @ V for M in self.Ms], axis=0)

    def _rmatmat(self, V):
        Vs = np.split(V, len(self.Ms))
    ...
    def to_dense(self):
        ...
        return np.concatenate(dense_Ms, axis=0)
...
class LazyDirectSum(LinearOperator):
    def __init__(self, Ms, multiplicities=None):
        ...
        shape = (sum(Mi.shape[0] * c for Mi, c in zip(Ms, multiplicities)), sum(Mi.shape[0] * c for Mi, c in zip(Ms, multiplicities)))
        super().__init__(None, shape)
        # self.dtype=Ms[0].dtype
    ...
    def to_dense(self):
        ...
        return scipy.linalg.block_diag(*Ms_all)
...
def lazy_direct_matmat(v, Ms, mults):
    ...
    for M, multiplicity in zip(Ms, mults):
        ...
        i = i_end
    y = np.concatenate(y, axis=0)  # concatenate over rep axis
    return y
...
class LazyShift(LinearOperator):
    ...
    def _matmat(self, V):
        return np.roll(V, self.k, axis=0)
